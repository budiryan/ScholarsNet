...
def search(search_query, search_category, db_cursor, num_result):
    ...
    if search_category == 'p':
        if score_rank_paper[0] == 0.0:
            ...
            to_be_returned = (title_rank[0:num_of_papers], rank_index_paper[0:num_of_papers], [], [])

    elif search_category == 'a':
        if score_rank_author[0] == 0.0:
            to_be_returned = ([], [], ["Your search did not match any author!!!"], [None])
        else:
            to_be_returned = ([], [], author_rank[0:num_of_authors], rank_index_author[0:num_of_authors])
    ...
    # search category for both
        ...
        if score_rank_paper[0] == 0.0 and score_rank_author[0] == 0.0:
            ...
            print("Your search did not match any paper!!!")
            to_be_returned = ("Your search did not match any paper!!!", [None], author_rank[0:num_of_authors], rank_index_author[0:num_of_authors])

        elif score_rank_paper[0] > 0.0 and score_rank_author[0] == 0:
            print("Your search did not match any author!!!")
            to_be_returned = (title_rank[0:num_of_papers], rank_index_paper[0:num_of_papers], ["Your search did not match any author!!!"], [None])

        elif score_rank_paper[0] > 0.0 and score_rank_author[0] > 0.0:
            to_be_returned = (title_rank[0:num_of_papers], rank_index_paper[0:num_of_papers], author_rank[0:num_of_authors], rank_index_author[0:num_of_authors])
